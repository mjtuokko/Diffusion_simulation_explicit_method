...
#    ax.set_yscale('log')
    ...
    for sub_folder in list_of_results:
        ...
        Path_to_data_folder=folder + sub_folder + "/3D_data/"
        Path_to_analytical_data_folder=folder + sub_folder + "/3D_analytical_data/" 
    ...
    #    data=np.loadtxt(new_folder_path + "3D_data.txt")
    #    c=data
    ...
    #    print(DX)
        for n in range(len(data_files)-2):
            ...
            filename=Path_to_data_folder+"3D_data_"+str(n)+".txt"
            value=np.loadtxt(filename)
            c.append(value)
            
            #add analytical value to the list
            filename=Path_to_analytical_data_folder +"3D_data_"+str(n)+".txt"
            value=np.loadtxt(filename)
        ...
        for i in range(len(T)):
            c_error.append((100*(c_analytical[i]-c[i])/c_analytical[i]))
       #     if(c_analytical[i]-c[i])>0:
       #         print("+")
       #     else:
       #         print("-")
        plt.style.use('own_plot_style.mplstyle')
        plt.plot(T,c,'o', label="$\Delta X$=%.3g" %DX + ", $\Delta T$=%.3g" %DT , linewidth=3, markersize=3)
        plt.plot(T,c_analytical,'o', label="Analytical: $\Delta X$=%.3g" %DX + ", $\Delta T$=%.3g" %DT , linewidth=1, markersize=3)
    plt.grid()
 #   plt.xlim(0, 0.006)
 #   plt.ylim(0,1.5)
    plt.legend(bbox_to_anchor=(1.12, 1.12))
    plt.ylabel("$c$:n suhteellinen virhe (%)")
    plt.xlabel("T")
