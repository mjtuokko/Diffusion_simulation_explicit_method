...
import main_program_1D as start
#make root window  
root = Tk()
...
def Button_Pressed():
    ...
    if(2*(Number_of_points-1)**2>=1/DT):
        messagebox.showinfo("Warning", "DT should be smaller. For " + str(Number_of_points) + " maximum value for DT is " + str(1/(2*(Number_of_points-1)**2)) )
        All_parameters_are_good=False
    if(All_parameters_are_good):
        ...
        start.start_program(Number_of_iterations,DT,Number_of_points,Number_of_points_analytical,Every_nth_save,make_1d_plots.get())
        Ready = Label(text = " Ready! \n See results in the result folder.")
        Ready.pack()
        messagebox.showinfo("Information", "Ready! \n See results in the result folder.")
    
l = Label(text = "Define parameters and answer the questions.")
...
c1 = tk.Checkbutton(root, text='I want 1D plots.',variable=make_1d_plots, onvalue=1, offvalue=0)
