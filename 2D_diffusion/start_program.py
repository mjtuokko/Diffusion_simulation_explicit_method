...
import main_program_2D as start
#make root window  
root = Tk()
...
def Button_Pressed():
    ...
    if(4*(Number_of_points-1)**2>=1/DT):
        messagebox.showinfo("Warning", "DT should be smaller. For " + str(Number_of_points) + " maximum value for DT is " + str(1/(4*(Number_of_points-1)**2)) )
        All_parameters_are_good=False
    if(All_parameters_are_good):
        ...
        start.start_program(Number_of_iterations,DT,Number_of_points,Number_of_points_analytical,Every_nth_save,make_2d_plots.get(),make_contour_plots.get(),make_1d_plots.get())
...
Every_nth_input = Text(root, height = 1,
                width = 5,
                bg = "white")
Every_nth_input.insert("end-1c",str(200))
...
Number_of_iterations_input = Text(root, height = 1,
                width = 7,
                bg = "white")
Number_of_iterations_input.insert("end-1c",str(10000))
...
c1 = tk.Checkbutton(root, text='I want 2D plots.',variable=make_2d_plots, onvalue=1, offvalue=0)
c2 = tk.Checkbutton(root, text='I want Contour plots.',variable=make_contour_plots, onvalue=1, offvalue=0)
c3 = tk.Checkbutton(root, text='I want 1D Cross section images.',variable=make_1d_plots, onvalue=1, offvalue=0)
